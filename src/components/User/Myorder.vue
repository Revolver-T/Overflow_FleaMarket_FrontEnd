  ...
  <div class="orderContainer">
    <el-tabs v-model="activeName">
        <el-tab-pane label="待支付" name="待支付">
          <el-row class="myOrderContainer" v-for="item in payedData" v-if="item.payed==1" :key="item.id">
          	<el-col :span="11">
          		<div class="proName">订单编号: {{item.orderId}}</div>
          		<div class="proDetail">创建日期: {{item.createTime}}</div>
          	</el-col>
          	<el-col :span="3">
          		<div class="proDetail">订单状态：{{item.payed==1?'未支付':'已支付'}}</div>
          		<div class="proDetail">商品数量：{{item.productCount}}</div>
          	</el-col>
            ...
            <el-col :span="4">
                ...
                <div @click="selectOrderById(item.orderId)">查看订单详情</div>
                <div @click="gotoPay(item)" v-if="item.payed==1">立即支付</div>
              </div>
            </el-col>
          </el-row>
        </el-tab-pane>
        ...
        <el-tab-pane label="已支付" name="已支付">
          <el-row class="myOrderContainer" v-for="item in payedData" v-if="item.payed==2" :key="item.orderId">
          	<el-col :span="11">
          		<div class="proName">订单编号: {{item.orderId}}</div>
          		<div class="proDetail">创建日期: {{item.createTime}}</div>
          	</el-col>
            ...
            <el-col :span="4">
            	<div class="proOperator" style="margin-left: 5px;">
                <div @click="selectOrderById(item.orderId)">查看订单详情</div>
                <div @click="deletePay(item.orderId)">删除订单</div>
                <div @click="clickReceving(item.id,item.orderId,4)">点击退款</div>
              </div>
            </el-col>
          </el-row>
        </el-tab-pane>
        ...
        <el-tab-pane label="待发货" name="待发货">
          ...
          <el-row class="cartContainer" v-for="item in waitGetData" :key="item.id" v-if="item.opstatus==1" style="margin: 5px 0;">
          	...
          	<el-col :span="3">
          		<div class="proCP">¥{{item.price}}</div>
          	</el-col>
          	<el-col :span="4">
          		<div class="proNumber">
                {{item.deal}}
        ...
        <el-tab-pane label="已发货" name="已发货">
          ...
          <el-row class="cartContainer" v-for="item in waitGetData" :key="item.id" v-if="item.opstatus==2" style="margin: 5px 0;">
            ...
            <el-col :span="4">
            	<div class="proOperator" style="margin-left: 5px;">
                <div @click="showProDetails(item.id)">查看</div>
                <div @click="clickReceving(item.id,3)">点击收货</div>
              </div>
            </el-col>
          </el-row>
        </el-tab-pane>
        ...
        <el-tab-pane label="已收货" name="已收货">
          ...
          <el-row class="cartContainer" v-for="item in waitGetData" :key="item.id" v-if="item.opstatus==3" style="margin: 5px 0;">
            ...
            <el-col :span="4">
            	<div class="proOperator" style="margin-left: 5px;">
                <div @click="showProDetails(item.id)">查看</div>
                <div @click="clickReceving(item.id,4)">申请退款</div>
              </div>
            </el-col>
          </el-row>
        </el-tab-pane>
      ...
      <el-dialog
        ...
        <div class="productInfo">
          <el-row v-for="item in order.products" :key="index">
            ...
            <el-col :span="2" class="infoTop"> <p>¥{{item.price}}</p></el-col>
          </el-row>
        </div>
        ...
        <el-row class="addressInfo">
          ...
          <el-col :span="15">{{order.receiver}}</el-col>
        </el-row>
        <el-row class="addressInfo">
          <el-col :span="3" :offset="1"> 联系方式 </el-col>
          <el-col :span="15">{{order.receiverPhone}}</el-col>
        </el-row>
      </el-dialog>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="商品售价" prop="price">
                  <el-input v-model.number="productData.price" placeholder="请填写商品售价" disabled></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8" :offset="1">
        ...
        this.$confirm('是否确定付款？', '提示', {
          ...
          this.$axios.post("/order/updateOrderByOid",{
            ...
            if(res.data == 1){
              ...
              //跳转支付界面，进行沙箱模拟支付
              setTimeout(res => {
                window.location.href = "http://localhost:9999/utils/orderPay?oid="+order.orderId+"&productTotal="+order.productTotal+"&remark="+order.remark;
              },500);
            }
          })
        });
